...
fn pending_offset_stops_at_pallet_cap_and_resumes_at_next_entry() {
	build_and_execute(|| {
		...
		// maximum.
		let (debt_offset, _) = simulate_pending_offset(DOT, PUSD, 1_000, 0);
		assert_eq!(debt_offset, 800);
		System::assert_has_event(
			crate::Event::PendingDepositOffsetApplied {
				collateral_id: DOT,
				stable_id: PUSD,
...
fn pending_backstop_rounds_down_at_the_minimum_balance_dead_zone() {
	build_and_execute(|| {
		...
		// One step consumed and the walk ended: nothing preservable remains.
		System::assert_has_event(
			crate::Event::PendingDepositOffsetApplied {
				collateral_id: DOT,
				stable_id: USDX,
...
/// allocation, spread across the FIFO oldest-first. The last depositor is
/// only partially consumed and keeps the rest pending.
///
/// The amounts divide evenly, so the per-step rule the code uses (price each
/// step against the remainders at its start) and the aggregate rule §6.8 of the
/// spec describes agree exactly here — this example does not discriminate
/// between them.
#[test]
fn example_9_pending_deposit_fifo_offset() {
	build_and_execute(|| {
		register_branch(DOT, PUSD, default_branch_config());
		// Alice, then Bob, then Cara.
		seed_pending(1, 300);
		seed_pending(2, 600);
		seed_pending(3, 500);
		...
		// 500 * 300/1_000 = 150, 500 * 600/1_000 = 300, 500 * 100/1_000 = 50.
		for (who, gain) in [(1, 150), (2, 300), (3, 50)] {
		...
		// Alice and Bob are fully consumed; Cara keeps 500 - 100 = 400 pending.
		assert!(!pending_contains(DOT, PUSD, 1));
		assert!(!pending_contains(DOT, PUSD, 2));
		assert_eq!(
			deposit_row(DOT, PUSD, 3).and_then(|row| row.pending_deposit).map(|p| p.amount),
			Some(400)
		);
	});
}

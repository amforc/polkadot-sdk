...
fn offsets_move_p_and_s_yield_moves_g() {
	build_with_default_market(|| {
		...
		assert_eq!(active_sums(0, 0).s_collateral, FixedU128::from_inner(160_000_000_000_000_000));
		assert_eq!(active_sums(0, 0).g_yield, FixedU128::zero());
		...
		assert_eq!(state.coords.p, FixedU128::from_rational(4, 5));
		assert_eq!(active_sums(0, 0).s_collateral, FixedU128::from_inner(160_000_000_000_000_000));
		assert_eq!(active_sums(0, 0).g_yield, FixedU128::from_inner(80_000_000_000_000_000));
		...
		assert_eq!(
			state.coords,
			Accumulators { p: FixedU128::from_rational(2, 5), epoch: 0, scale: 0 }
		);
		assert_eq!(active_sums(0, 0).s_collateral, FixedU128::from_inner(480_000_000_000_000_000));
		assert_eq!(active_sums(0, 0).g_yield, FixedU128::from_inner(80_000_000_000_000_000));
	});
}
...
fn pending_accumulators_reset_on_depletion_not_on_an_empty_queue() {
	build_with_default_market(|| {
		...
		assert_eq!(pending_sums(0, 0).s_collateral, FixedU128::from_rational(1, 4));
		...
		assert_eq!(
			...
			Accumulators { p: FixedU128::from_rational(1, 2), epoch: 0, scale: 0 }
		);
		assert_eq!(pending_sums(0, 0).s_collateral, FixedU128::from_rational(1, 4));
		...
		assert_eq!(pending_sums(1, 0).s_collateral, FixedU128::zero());
		// The closing epoch keeps its row: 0.25 + 80 * (0.5/400) = 0.35, the
		// window user 2 still claims through.
		assert_eq!(pending_sums(0, 0).s_collateral, FixedU128::from_rational(35, 100));
	});
}

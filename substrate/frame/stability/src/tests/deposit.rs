...
fn deposit_moves_funds_and_queues_pending() {
	build_and_execute(|| {
		...
		// Deposited at block 1 with a 5-block entry delay.
		assert_eq!(pending.activatable_at, 6);
...
fn second_deposit_merges_and_resets_delay() {
	build_and_execute(|| {
		...
		// The merge restarts the whole amount's delay: block 3 + 5.
		assert_eq!(pending.activatable_at, 8);
...
fn deposit_auto_activates_matured_pending() {
	build_and_execute(|| {
		...
		assert_ok!(deposit(1, DOT, PUSD, 400));
		// Matured at block 6; the next deposit activates it first.
		run_to_block(6);
		assert_ok!(deposit(1, DOT, PUSD, 300));
		...
		// Fresh delay for the new amount: block 6 + 5.
		assert_eq!(pending.activatable_at, 11);

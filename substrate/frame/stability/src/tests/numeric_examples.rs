//! The Stability Pool examples from `numeric-examples.md`, at the parameters
//! its shared assumptions fix: 1 DOT = 2 pUSD, `final_recovery_bonus_buffer`
//! 1%, `redistribution_penalty` 10%.
//!
//! These are reference numbers: the spec's arithmetic, reproduced against the
//! real pallet. Where an example's figures are only reachable through a
//! particular history, the setup that gets there is spelled out rather than
//! poked into storage.

use crate::{mock::*, Error};

/// The examples' vaults sit at CR 120%, above the default 110% MCR, so
...
/// Numeric example 5.1: an incoming deposit meets a `FinalRecovery` head at
/// CR >= 100%, so it is spent on the recovery instead of queueing, and the
/// depositor is paid in collateral rather than a pool position.
#[test]
fn example_5_1_incoming_deposit_recovery_offset_is_accepted() {
	build_and_execute(|| {
...
/// Numeric example 5.2: the same deposit against a head below par is refused
/// outright — no burn, no pending position.
#[test]
fn example_5_2_incoming_deposit_below_par_is_rejected() {
	build_and_execute(|| {
		register_branch(DOT, PUSD, example_branch_config());
		// 5_000 of debt against 2_000 DOT = 4_000 pUSD, an 80% CR.
		open_at_120_percent(5, 2_000, 2_000);
		// Draw the CR down to the example's 80% by dropping the price.
...
/// Numeric example 6: an active-pool recovery offset, and the depositor
/// realization it implies.
#[test]
fn example_6_active_pool_recovery_offset_and_realization() {
	build_and_execute(|| {
		register_branch(DOT, PUSD, example_branch_config());
		open_at_120_percent(5, 3_000, 5_000);

		// 10_000 of active deposits, of which the example follows 1_000.
		seed_active(1, 1_000);
		seed_active(2, 9_000);
...
/// Numeric example 10: a liquidation offset, then yield routed over the
/// shrunken pool, and one depositor realizing all three quantities.
#[test]
fn example_10_offset_then_yield_then_realization() {
	build_and_execute(|| {
...
/// Numeric example 12: a full depletion at a compounded `P`, which closes the
/// epoch and resets the accumulators while leaving the old epoch's gains
/// claimable.
///
/// The example states `P = 0.42` at `epoch 7, scale 3`. The epoch and scale are
/// labels — the arithmetic depends only on `P` and the pool total — so this
/// reaches `P = 0.6 * 0.7 = 0.42` with two collateral-free offsets and asserts
/// the epoch increments rather than that it reaches 8.
#[test]
fn example_12_full_depletion_and_epoch_transition() {
	build_and_execute(|| {
		...
		// The example's depositor joins at P = 0.42 with 600, and a third tops
		// the pool up to the example's 1_500.
		seed_active(2, 600);
		seed_active(3, 480);

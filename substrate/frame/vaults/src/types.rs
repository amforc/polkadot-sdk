...
pub enum BranchConfigUpdate<Balance> {
	...
	/// Replaces the market's liquidation configuration.
	Liquidation(LiquidationConfig<Balance>),
	/// Sets the collateral penalty applied to redistributed debt.
...
impl<Balance: PartialOrd + Copy> BranchConfigUpdate<Balance> {
	...
	pub const fn apply_to(self, config: &mut BranchConfig<Balance>) {
		match self {
			...
			Self::Liquidation(v) => config.liquidation = v,
...
impl<Balance: PartialOrd + Copy + Zero> BranchConfigBounds<Balance> {
	...
	pub fn permits(&self, config: &BranchConfig<Balance>) -> bool {
		config.minimum_collateralization_ratio >= self.min_minimum_collateralization_ratio &&
			...
			config.liquidation.offset_penalty <= config.liquidation.redistribution_penalty &&

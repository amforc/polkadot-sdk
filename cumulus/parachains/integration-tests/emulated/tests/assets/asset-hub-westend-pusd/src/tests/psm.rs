...
fn create_pusd_psm(max_debt: Balance) -> (Location, Location) {
	let internal = TrustBackedAssetLocation::convert_back(&PUSD_ID)
...
fn psm_debt_consumes_and_releases_vault_headroom() {
	AssetHubWestend::execute_with(|| {
		...
		// Another 150 pUSD fits the 1_000 cap while the PSM holds no debt.
		hypothetically_ok!(borrow(150 * PUSD));
		...
		// The same 150 pUSD still fits on the vault side alone (600 + 150), but
		// live PSM debt of 300 leaves vaults only 700.
		assert_noop!(
			borrow(150 * PUSD),
